...
Converted by Sören Dittmer, Clemens Karner and Anna Breger (through project FWF T1307-N/Anna Breger) from the original Python tensorflow implementation written by David Neumann and from the original MATLAB implementation written by Rafael Reisenhofer.
...
def haarpsi(ref, deg, C, α,preprocess_with_subsampling=True):
    ...
    assert torch.all(deg >= 0) and torch.all(deg <= 1), "The images must be in the range [0, 1]"
    ...
    assert ref.shape[1] == deg.shape[1], "The images must both have the same number of color channels."
    ...
    if is_color_image:
        ...
        deg_y = 0.299 * deg[:, 0, :, :] + 0.587 * deg[:, 1, :, :] + 0.114 * deg[:, 2, :, :]
        ref_i = 0.596 * ref[:, 0, :, :] - 0.274 * ref[:, 1, :, :] - 0.322 * ref[:, 2, :, :]
    ...
    n_scales = 3
    coeffs_ref_y = _haar_wavelet_decompose(ref_y, n_scales) # n_scales x n_samples x 1 x height x width
    ...
    local_similarities = torch.zeros(n_channels, n_samples, 1, height, width, device=ref_y.device)
    weights = torch.zeros(n_channels, n_samples, 1, height, width, device=ref_y.device)

    for orientation in [0, 1]:
        weights[orientation] = _get_weights_for_orientation(coeffs_deg_y, coeffs_ref_y, n_scales, orientation, n_samples, height, width)
        local_similarities[orientation] = _get_local_similarity_for_orientation(C, coeffs_deg_y, coeffs_ref_y, n_scales, orientation)

    if is_color_image:
        similarity_i = (2 * coefficients_ref_i * coefficients_deg_i + C) / (coefficients_ref_i**2 + coefficients_deg_i**2 + C)
        similarity_q = (2 * coefficients_ref_q * coefficients_deg_q + C) / (coefficients_ref_q**2 + coefficients_deg_q**2 + C)
    ...
    assert local_similarities.shape == (n_channels, n_samples, 1, height, width), local_similarities.shape
    ...
    assert local_similarities.shape == (n_samples, n_channels, height, width), local_similarities.shape
    assert weights.shape == (n_samples, n_channels, height, width), weights.shape
    return similarity, local_similarities, weights


def _get_local_similarity_for_orientation(C, coeffs_deg_y, coeffs_ref_y, n_scales, orientation):
    n_samples = coeffs_ref_y.shape[1]
    height, width = coeffs_ref_y.shape[-2:]
    coeffs_ref_y_magnitude = coeffs_ref_y.abs()[(orientation * n_scales, 1 + orientation * n_scales), :, :]
    coeffs_deg_y_magnitude = coeffs_deg_y.abs()[(orientation * n_scales, 1 + orientation * n_scales), :, :]
    ...
    assert local_similarity.shape == (n_samples, 1, height, width), local_similarity.shape
    return local_similarity


def _get_weights_for_orientation(coeffs_deg_y, coeffs_ref_y, n_scales, orientation, n_samples, height, width):
    maxs = torch.maximum(coeffs_ref_y[2 + orientation * n_scales].abs(), coeffs_deg_y[2 + orientation * n_scales].abs())
...
def _convolve2d(data, kernel):
    ...
    res = torch.nn.functional.interpolate(res, (width, height), mode='nearest', align_corners=None)
